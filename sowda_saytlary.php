...
<ul style="display:flex; flex-direction:column">
  <?php 
    ...
    $sayfa=@intval($_GET['s']);
    if(!$sayfa){$sayfa=1;}
    $toplam=mysqli_num_rows(mysqli_query($baglan,"select * from ulanyjy_reklama WHERE aktif=1 AND category='sowda'"));

    $limit=5;
    $sayfa_sayisi=ceil($toplam/$limit);
    if($sayfa>$sayfa_sayisi){$sayfa=1;}
    $goster=$sayfa*$limit-$limit;
    $kontrol=mysqli_query($baglan,"select * from ulanyjy_reklama WHERE aktif=1 AND category='sowda' order by id DESC limit $goster,$limit");  
...
if ($sayfa>10) {
  $sonraki = $sayfa-10;
  echo "<div><a href = 'sowda_saytlary.php?s=$sonraki'>-10</a></div>";
}
      if($sayfa > 1){
        $onceki = $sayfa -1;
        echo "<div> <a href='sowda_saytlary.php?s=$onceki'>Yza</a> </div>";
      }
...
for ($i=$sayfa-$gorunen; $i < $sayfa+$gorunen+1; $i++) { 
  if($i>0 and $i <=$sayfa_sayisi){
    ...
    echo "<div><a href='sowda_saytlary.php?s=$i'>$i</a></div>";
  }}
}
  
if ($sayfa != $sayfa_sayisi ) {
  $sonraki = $sayfa+1;
  echo "<div><a href = 'sowda_saytlary.php?s=$sonraki'>One</a></div>";
}
